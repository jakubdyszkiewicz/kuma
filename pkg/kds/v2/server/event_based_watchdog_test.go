...
var _ = Describe("Event Based Watchdog", func() {
	...
	It("should reconcile on the first flush", func() {
		...
		Eventually(func(g Gomega) {
			g.Expect(*test_metrics.FindMetric(metrics, "kds_delta_generation", "reason", ReasonResync).Summary.SampleCount).To(BeEquivalentTo(1))
		}, "10s", "50ms")
	})
	...
	It("should reconcile on the events flush", func() {
		...
		Eventually(func(g Gomega) {
			g.Expect(*test_metrics.FindMetric(metrics, "kds_delta_generation", "reason", ReasonEvent).Summary.SampleCount).To(BeEquivalentTo(1))
		}, "10s", "50ms")
	})
	...
	It("should reconcile on the full resync", func() {
		...
		Expect(changedResTypes).To(Equal(watchdog.ProvidedTypes))
		Eventually(func(g Gomega) {
			g.Expect(*test_metrics.FindMetric(metrics, "kds_delta_generation", "reason", ReasonResync).Summary.SampleCount).To(BeEquivalentTo(2))
		}, "10s", "50ms")
	})
}, Ordered)
